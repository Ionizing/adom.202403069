...
class AveragedResult:
    ...
    def plot_phonon_photon_contribution(self,
        ...
        if iframe <= 0:
            ...
            contributions = contributions[iframe-1, :, :]
    ...
    def plot_valley_population(self, pngfname="valley_populations.png"):
        ...
        axs[0].plot(time, pops[:,1:], ls="-",  label=valleys_with_spin[1:])
        axs[0].plot(time, pops[:,0 ], ls="--", label=valleys_with_spin[0])
        ...
        polarization = (pops[:,2] - pops[:,1]) / (pops[:,2] + pops[:,1] + 1E-6)
        axs[1].plot(time, polarization)
